...
分析用データマート構築・データ基盤整備のエンジニアとして、大手企業のデータ活用を支援しています。  
分析寄りのデータエンジニアとして、データ変換から分析・施策提案まで一貫したソリューションを提供します。

<p style="margin-top: 15px; font-size: 0.95rem; color: #6b7280; line-height: 1.8;">
    「データを活用してチームの成果に貢献する」——これが一貫したキャリアの軸です。<br><br>
    <strong>営業事務時代（2011-2013年）</strong>：Excel VBAで3つのデータソースを統合する在庫管理システムを自作。対象商品の売上前年比120〜130%を達成しました。<br><br>
    <strong>旅行代理店時代（2014-2019年）</strong>：GA分析をもとにメールマガジンのコンテンツを改善し、反応率を通常比1.3倍に複数回引き上げました。この「分析→施策→結果確認」のサイクル全体に関わる経験が、データエンジニアを目指す出発点になりました。<br><br>
    <strong>業務委託期間中（2022-2023年）</strong>：自分が整備したデータがクライアントに活用されない現実に直面し、「分析以前に、データ基盤そのものが必要だ」と気づきエンジニアへの転換を決意しました。<br><br>
    <strong>データエンジニアとして（2024年1月〜現在）</strong>：大手企業向けの分析用データマート実装に従事。主な実績は以下のとおりです。
    <ul style="margin-top: 10px; margin-bottom: 10px; padding-left: 1.5rem;">
        <li>マーケティングタグ管理基盤の統合移行（2,724項目テスト、タグ発火率99.8%達成）</li>
        <li>BIデータマート実装（分析リードタイム96%削減）</li>
        <li>SFCC APIリトライ処理実装、空港事業向け移動データSQL作成・異常値補正、IPOCA分析用データマート新規構築（lake層直接処理・10空港対応）</li>
    </ul>
    次のキャリアでは、BtoB SaaS企業において社内チームを支援するデータ基盤を構築しながら、分析・施策提案まで一貫して関わる環境を目指しています。
</p>
...
旅行会社時代、Web制作担当として上司との二人チームでデータ分析から施策立案・Webコンテンツ改善まで担当し、前週比1.3倍の反応率を継続的に達成した経験から、「データ分析→施策実行→結果確認→チームへの貢献」というサイクルに大きなやりがいを感じました。しかし、その後のマーケティング補助の仕事で、私が整えたデータがクライアント企業で全く使われていない現実に直面しました。理由は、データの使い方が分からない、分析方法が分からない、優先順位が低い、というものでした。
...
- 上司との二人チームでWebコーディング・デザイン・データ分析を担当
- MySQLからSQLでデータを抽出し、Pythonで回遊率や予約予測を可視化
- マーケティングチームと施策を講じ、ページのコンテンツやデザインを編集
- データドリブンなWeb制作により、流入率や予約率が如実に変化。「このバナーに変えたら回遊が増えた」「この特集を組んだら予約が増えた」という喜びを実感
- 週1回のメールマガジンでは、ユーザーの行動データを分析して自分でツアーを選定して配信。独自選定ツアーのメルマガで前週比1.3倍の反応率を継続的に達成（通常は0.8〜1.1倍）
- 上司から「佐藤さんが作ったメルマガ、反応良いよー！」と評価していただき、今でも印象に残っている
- この「データ分析→施策実行→結果確認→評価」というサイクル全体に関わることで、自分の貢献が可視化され、社内チームの成果に繋がっていることを実感
- **データで成果を出し、チームから感謝された経験が、「データで貢献したい」という軸の出発点となった**
- SQLは2017年から、Pythonは2018年から（pandas、Jupyter Notebook環境）、どちらも独学で習得。「できることが増えると、提案できることも増える」という感覚が学ぶ動力となり、後のデータエンジニアキャリアの礎となった
...
3. **フィードバックが得られる機会がない**
   - **現職の状況**：ミスの指摘や注意の言葉はしっかり受け止める、成長を自己分析して感じることはできるが、第三者目線から成長と貢献を感じたい。「データ基盤を整備した」という技術的な貢献はしているはずだが、誰からも評価していただけない
   - **自分にとって重要な理由**：一緒に働く人から評価していただくことが、仕事のモチベーションになる。大手旅行代理店時代の「佐藤さんが作ったメルマガ、反応良いよー！」という言葉が、最も嬉しい瞬間だった
...
- **データドリブンなWeb制作・施策実行・効果測定**（前週比1.3倍のメルマガ反応率を継続的に達成）
...
### 10. 📊 dbt学習プロジェクト - 実務SQLをdbt化した10日間
**役割**: 個人学習プロジェクト  
**期間**: 2024年12月（10日間集中学習）

## 背景

実務で保守していた300行の複雑なSQLが、データ量増加により処理時間2時間・手動テストに1日を要する状況に。

**dbt導入により処理時間を96%削減し、テストを完全自動化**。このプロジェクトは、その学習プロセスと実装ノウハウの記録です。

- **課題**: 
  - 300行の単一SQLファイルで保守性が低く、変更時の影響範囲が不明
  - 処理時間が2時間かかり、データ量増加に伴いさらに増加する見込み
  - 手動テストに1日を要し、Excelで定義書を手動更新（半日）
  - 変更時の影響確認に3日、新メンバーへの引き継ぎに1週間
- **技術実装**: 
  - 300行の単一SQLを12モデルに分解し、階層構造（staging/intermediate/marts）で設計
  - incrementalモデルで差分更新を実装、処理時間を96%削減（2時間→6分）
  - 27テストでデータ品質を保証（カスタムテスト + dbt_expectations）
  - 3環境構成（dev/stg/prd）とGitHub Actions CI/CDを構築
  - dbtドキュメント自動生成とGitHub Pages公開
- **成果**: 
  - 処理時間を2時間から6分に短縮（96%削減）
  - テストを手動1日から自動5分に短縮（95%削減）
  - ドキュメント更新を手動半日から自動0分に短縮（100%削減）
  - 変更時の影響確認を3日から5分に短縮（99%削減、Lineage Graph活用）
  - 新メンバー引き継ぎを1週間から5分に短縮（99%削減）
- **技術**: dbt 1.11.0, BigQuery, GitHub Actions, GitHub Pages, dbt_expectations
- **成果物**: 
  ...
  - 📝 **Qiita記事**: [実務SQLをdbt化した10日間 - データエンジニアが語る、ビジネス視点を失わない技術習得法](https://qiita.com/sato1046/items/7d3c0e1df82d6d8f0d2e)

![Lineage Graph](docs/images/lineage_graph.png)
*モデル間の依存関係を自動生成*

![dbt Docs](docs/images/dbt_docs.png)
*dbtドキュメントによる自動ドキュメント生成*
...
### 1. エンドツーエンドのデータ基盤構築力
- SFCC API連携基盤開発：OAuth 2.0認証、バイナリサーチによる適応的期間分割、10万件単位のバッチ処理、150項目以上のカラムマッピングを実装
- BIデータマート実装：300行超の大規模SQLで12パターンの多次元集計を実現、分析リードタイムを2日から30分に短縮（96%削減）
- ETLパイプライン構築：SFTP連携、API連携、日次バッチ自動化により月40時間の作業時間削減を達成
- 過去の経験：在庫管理システム構築で前年比120%売上達成、Web制作・データ分析・メルマガ選定で前週比1.3倍反応率を継続的に実現

### 2. 先読み力とリスク管理能力
- API制限を想定したリトライ処理実装：3回リトライ・120秒間隔設定、全エラーパターンのテスト実施により運用負荷を軽減
- Response Entity Too Largeエラーの自動処理：最大5回の期間再分割により、大容量データ取得の安定化を実現
- 大規模タグ移行の計画的実行：2,724項目の網羅的テストを実施し、タグ発火率99.8%を達成
- データ連携の確実性向上：401認証エラー、0件エラーもリトライ対象とし、一時的なエラーからの自動復旧機能を実現

### 3. 高い情報処理能力と複雑な問題の構造化力
- 大規模クエリ開発：700行超のSQLを10段階以上のCTEに分解、5段階CTE処理による重複排除とデータ正規化を実現
- 複雑なマッピング実装：150項目以上のカラムマッピング辞書（キャメルケース→スネークケース）を正確に構築
- GPS異常値検出アルゴリズム：Haversine公式による球面距離計算、V字型異常パターン検出、適応的傾向分析を実装
- マーケティングタグ統合移行：2,400項目の変数・トリガー・タグを体系的に整理し、4ブランド並行移行を完遂

### 4. 自律的な開発推進力
- フルリモート環境で1年11ヶ月、大手企業向けデータマート実装を主体的に推進
- AI開発支援ツール（Claude、Cursor）を積極的に活用し、開発効率を継続的に向上
- 外部ベンダーとの技術的コミュニケーションを主体で担当：質問文書・回答文書の作成により円滑な連携を実現
- リカバリー対応を実作業時間1時間で完了：Embulk設定修正により787,303レコードの欠損データを正常取込

### 5. 論理的問題解決力とデータ品質へのこだわり
- タグ発火率の大幅改善：95%から99.8%に向上、エラー率を10%から0.2%に削減
- 網羅的テスト実施：2,724項目のテスト項目を計画・実行し、4ブランドの並行移行を完遂
- データ連携の安定稼働率99.9%達成：学習管理システムとBigQueryのETLパイプライン構築で実現
- 過去の経験：1ヶ月1,000万円以上の請求を間違いなく処理し、信頼関係を構築（サントリーフラワーズ）

### 6. マーケティング支援とチーム貢献への強いモチベーション
- マーケティングタグ管理基盤統合：2,400項目の変数・トリガー・タグ移行により、マーケティングチームがデータ活用しやすい環境を整備
- GA4イベント設計・実装：11種類のEコマースイベントを設計し、分析基盤を強化
- 同僚への情報提供：調べた情報を積極的に共有し、チームの業務効率化に貢献
- 過去の経験：「前任者ができないことをやってくれた！本当にありがとう！」と通販事業部から評価
...
✅ マルチソースデータの統合と正規化  
✅ スケーラブルなデータアーキテクチャ設計  
✅ **データ分析から施策提案まで一貫した支援**（大手旅行代理店時代のWeb制作・データ分析経験：前週比1.3倍の反応率を継続的に達成）  
