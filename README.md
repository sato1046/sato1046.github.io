...
- 大手不動産企業空港事業部（クライアント様）向けに、特定地域エリア11空港・駅に特化したIPOCAレポートSQLを5種類作成。また、移動データにおける物理的にあり得ない高速移動（時速1000km超など）のノイズデータを検出・補正する処理を実装
- **課題**: 
  - 前回プロジェクトで作成したIPOCAレポートSQLを、空港事業部（クライアント様）向けに特定地域エリアに特化したレポート用SQLにカスタマイズする必要があった
...
- **技術実装**: 
  ...
  - 対象空港の絞り込み（特定地域エリア11空港・駅）、期間フィルタリング（2023年4月1日～2025年3月31日）
