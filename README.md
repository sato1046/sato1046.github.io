...
大学卒業後、派遣などで営業事務や経理を経験。営業事務にてExcelを学び、迅速正確なデータ集計や問題点の発見と課題解決、業務の効率化を行い、需要と供給のバランスを取ることで売上に貢献。

その後、株式会社エイチ・アイ・エスのWEB制作において、週次メールマガジンの企画・制作を担当。ユーザー視点でのコンテンツ選定により、独自選定ツアーのメルマガで前週比1.3倍の反応率を達成（通常は0.8〜1.1倍）。配信後のアクセス数・予約数をGoogle Analyticsで分析し、効果測定とコンテンツ改善を実施。また、SQLやPythonを活用したデータ分析により、効果的なUX/UIデザインやバナー制作を行い、売上向上に貢献。

結婚出産を機に、業務委託としてWebマーケティング会社でマーケティングアシスタントを担当。手入力で2時間かかっていた資料作成業務を、Excel数式・関数の活用により30分に短縮（75%削減）。クライアント向けSEO対策資料作成や検索キーワード分析を通じて、マーケティング支援に貢献。

2024年1月より株式会社FUTURE MATRIXにてデータエンジニアとして従事。大手企業向けのデータ基盤構築を中心に、Treasure DataやBigQueryを活用したETLパイプライン開発、複雑なSQL開発（700行超のクエリ実装）、Digdagによるワークフロー自動化などを担当。特に、アパレル4ブランドのマーケティングタグ管理基盤の大規模統合移行プロジェクト（Tealium→GTM、2,400以上の設定項目、2,724項目の網羅的テスト実施）では、マッピングシート作成、カスタムHTML/JavaScript開発、タグ発火テスト（発火率99.8%達成）を担当し、マーケティングチームのデータ活用基盤を整備。SFCC APIとの連携やGA4タグ実装、Python/Cloud Functionsを用いたサーバーレスアーキテクチャの構築など、幅広いデータエンジニアリング業務で顧客のDX推進に貢献。
...
### 【データエンジニアリング】
- **Python**（pandas、numpy、seaborn、selenium、requests、cryptography）の実務経験（3年以上）
- **SQL**（CTE、Window関数、複雑なJOIN、UNION、パーティション処理、700行超の大規模クエリ開発）の実務経験（6年以上）
- **JavaScript**（UDF開発、カスタムアルゴリズム実装、GTMカスタムHTML開発）の実務経験
- **クラウドプラットフォーム**（GCP：BigQuery、Cloud Functions、Secret Manager）の構築・運用経験
- **データ基盤**（Treasure Data、Digdag）の実装・運用経験
- **ETLパイプライン開発**（SFTP連携、API連携、データクレンジング）の実務経験

### 【マーケティング支援】
- **マーケティングツール**（GTM、GA4、Tealium、Braze、LINE STAFF START、Silver Egg、Repro）の実装・移行経験
- **大規模タグ管理基盤の統合移行**（2,400項目、4ブランド並行、2,724項目の網羅的テスト実施）
- **GA4イベントトラッキング設計・実装**（11種類のEコマースイベント）
- **メールマーケティング施策の企画・実行・効果測定**（前週比1.3倍の反応率達成）
- **Google Analytics、キーワードプランナーを活用したデータ分析**
- **SEO対策資料作成、検索キーワード分析によるクライアント支援**

### 【業務効率化・データ分析】
- **地理空間データ分析・異常値検出アルゴリズムの開発経験**
- **AI開発支援ツール**（Claude、Cursor、GitHub Copilot）を活用した開発効率化・コード品質向上の実務経験
- **問題発見力・課題解決力・業務の効率化**（75%の作業時間削減実績）
- **データ整理・集計・分析の経験**
- **Excel VBA、数式・関数を活用した業務自動化**

### 【その他】
- **Webマーケティング業界の実務経験**
- **Webディレクションのアシスタントの実務経験**
- **Webデザインに関する実務経験**（Photoshop、Fireworks、HTML/CSS）
