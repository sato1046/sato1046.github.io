...
**旅行代理店時代（2014-2019年）** - 💡 理想の働き方の原点：Google Analyticsで閲覧数・回遊率を分析し、Webページのデザインやメールマガジンのコンテンツを改善。独自選定ツアーのメルマガで**前週比1.3倍の反応率**を複数回達成（通常は0.8〜1.1倍）。上司から「佐藤さんが作ったメルマガの反応良いよー！」と評価していただいた経験が、私にとって最も嬉しい瞬間でした。この「データ分析→施策実行→結果確認→評価」というサイクル全体に関わることで、自分の貢献が可視化され、社内チームの成果に繋がっていることを実感できました。SQLでこれまで整理しきれていなかった分析データを抽出し、Pythonでデータ整形・集計。様々な予測を立ててWebデザインに活用し、データドリブンなWebデザインにより、閲覧数や回遊率が如実に変化。マーケティング施策に活かす面白さを知り、「もっとデータに深く関わりたい」と思うようになりました。
...
**データエンジニア時代（2024年1月～現在）**：データエンジニアとして1年11ヶ月、大手企業向けの分析用データマート実装に従事。アパレル4ブランドのマーケティングタグ管理基盤の統合移行（2,724項目のテスト実施、タグ発火率99.8%達成）、BIデータマート実装（分析リードタイム96%削減）、SFCC Products API連携による大規模商品データ取込基盤構築（Salesforce社への技術質問対応を主体で担当）、SFCC Customer API連携のリトライ処理実装、モバイルアプリデータリカバリー対応、大手不動産企業空港事業部向けIPOCAレポートSQL作成・移動データ異常値補正、backup_data WFのパラレル実行化対応など、技術的なスキルを習得しました。
...
- 上司から「佐藤さんが作ったメルマガの反応良いよー！」と評価していただいた経験が、私にとって最も嬉しい瞬間でした
...
- 大手旅行代理店時代の「データ分析→施策→結果が見える→評価」というサイクルをもう一度経験したい
...
2. **貢献実感がない**
   ...
   - **大手旅行代理店時代との違い**：大手旅行代理店では、「データ分析→施策実行→結果が数値で見える→上司から評価していただく」という完結したサイクルがあった。自分の貢献が可視化され、上司から「佐藤さんのメルマガの反応良いよー！」と評価していただいた

3. **フィードバックが得られる機会がない**
   - **現職の状況**：ミスの指摘や注意の言葉は来るが、ポジティブなフィードバックがほとんどない。「データ基盤を整備した」という技術的な貢献はしているはずだが、誰からも評価していただけない
   - **自分にとって重要な理由**：自己肯定感が低いため、評価していただくことで「自分の存在価値」を実感できる。大手旅行代理店時代の「佐藤さんのおかげで反応良いよー！」という言葉が、最も嬉しい瞬間だった。一緒に働く人から評価していただくことが、仕事のモチベーションになる
...
- **「社内チームへの直接貢献がしたい」**：エンドユーザーの反応よりも、一緒に働く人から「佐藤さんのおかげで」と評価していただきたい。マーケティングチームや営業チームが「データのおかげで分析が楽になった」「施策が成功した」と喜んでくれる環境
...
- **技術実装**: 
  ...
  - **Salesforce社への技術質問文書の作成（主体で担当）**: 大量データ取得における技術的課題を明確化し、質問事項を項目分け（大量データ取得、API制限、推奨実装方法など）
  - **Salesforce社からの質問に対する回答文書の作成（主体で担当）**: 実装コードから確認できる内容を明確に記載し、やり取りの文章も主体で作成・管理
- **成果**: 
  ...
  - 外部ベンダー（Salesforce社）との技術的なコミュニケーションを主体で担当
...
### 10. 🏢 大手不動産企業【空港事業部】クライアント様データ連携・データ補正対応
**役割**: データエンジニア（実装担当）  
**期間**: 2025年11月初旬～11月末  
**チーム規模**: 2名

- 大手不動産企業空港事業部（クライアント様）向けに、北海道エリア11空港・駅に特化したIPOCAレポートSQLを5種類作成。また、移動データにおける物理的にあり得ない高速移動（時速1000km超など）のノイズデータを検出・補正する処理を実装
- **課題**: 
  - MEC_FM-1で作成したIPOCAレポートSQLを、空港事業部（クライアント様）向けに北海道エリアに特化したレポート用SQLにカスタマイズする必要があった
  - クライアント様の移動データにおいて、GPS測位エラーやデータ品質の問題により、物理的にあり得ない高速移動が記録されている
- **技術実装**: 
  - BigQuery SQLによる5種類のレポート作成（behavior-airport、behavior-airport_fy2023_2024、chara-airport、residence-airport、visittime-airport）
  - 対象空港の絞り込み（北海道エリア11空港・駅）、期間フィルタリング（2023年4月1日～2025年3月31日）
  - chara-airportレポートから40項目を削除（EXCEPT句を使用）
  - 移動データ異常値補正ロジックの実装（時速1000km以上、移動距離60km未満かつ時速320km超の条件で補正）
  - クライアントとの複数回のやり取りを経て、補正条件を最適化（距離条件を175km→60kmに調整）
- **成果**: 
  - 北海道エリアに特化した5種類のレポートSQLを完成
  - 移動データの異常値を自動検出・補正し、データ品質を向上
  - クライアント要望への迅速な対応により、継続的なロジック改善を実現
- **技術**: Google BigQuery, Standard SQL, 地理空間データ処理, 時系列データ補正ロジック

### 11. 🔧 タカラトミー様 backup_data WFパラレル実行化対応
**役割**: データエンジニア（実装担当）  
**期間**: 2025年11月6日～11月10日  
**チーム規模**: 2名

- タカラトミー様のbackup_data WFについて、Trino用にWFを再調整し、パラレル実行に変更する作業を実施
- **課題**: 
  - backup_WFをパラレル実行に変更する必要があった
  - 対象を削る（analysis/l3/weblogは削除）必要があった
  - パラレル処理への変更によりメモリ不足になった場合のHive化対応が必要
- **技術実装**: 
  - Digdagワークフローに`_parallel: true`を追加してパラレル実行を有効化
  - 対象データベースの調整（analysis、l3、weblogを削除）
  - config/variables.dig内のテーブル順序をL1~L3で整理
  - メモリ検証を実施（最大使用メモリ404MB、メモリ上限60GB/ジョブ）
- **成果**: 
  - パラレル実行への変更を完了
  - メモリ検証によりTrinoで問題なし（Hive化不要）を確認
  - 全テーブルでの検証が成功
  - 定義書（TD設定仕様書ver16）を更新
- **技術**: Treasure Data, Digdag, Trino, Hive, Presto SQL, S3
